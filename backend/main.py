...
if __name__ == "__main__":
    ...
    logger = logging.getLogger("notilus.backend")
    port = int(os.getenv("PORT", 8000))
    host = "127.0.0.1"
    is_frozen = bool(getattr(sys, "frozen", False))
    reload_enabled = not is_frozen

    logger.info(
        "Backend startup mode=%s host=%s port=%s cwd=%s",
        "frozen" if is_frozen else "script",
        host,
        port,
        os.getcwd(),
    )
    ...
    try:
        # In packaged mode (PyInstaller), avoid reloader/import-string startup
        # to prevent child-process boot issues.
        if is_frozen:
            uvicorn.run(
                app,
                host=host,
        ...
        logger.exception("Backend startup failed: %s", exc)
        raise SystemExit(1)
